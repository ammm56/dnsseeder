...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/ammm56/dnsseeder)

DNSSeeder exposes a list of known peers to any new peer joining the Hoosat network via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the htnd node
...
disk in a json file, so on subsequent start ups the htnd node specified with
...
- Launch a htnd node for the DNSSeeder to connect to
...
```bash
$ git clone https://github.com/ammm56/dnsseeder $GOPATH/src/github.com/ammm56/dnsseeder
$ cd $GOPATH/src/github.com/ammm56/dnsseeder
$ go install .
```
...
To create a working set-up where the DNSSeeder can provide IPs to htnd instances, set the following DNS records:
